...
/* harmony export (immutable) */ __webpack_exports__["b"] = test_sites;
...
const extractObject = {
    ...
    setPref: function(name, value) {
       return bgPage.setPref(name, value);
    }
}
/* unused harmony export extractObject */
...
"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return CryptoJS; });
/*
CryptoJS v3.0.2
...
"use strict";
/* harmony export (immutable) */ __webpack_exports__["a"] = cropUrl;
/* harmony export (immutable) */ __webpack_exports__["c"] = isURL;
/* unused harmony export escapeRegExp */
/* harmony export (immutable) */ __webpack_exports__["b"] = hasSpecialChar;
/* harmony export (immutable) */ __webpack_exports__["d"] = testRegExp;
...
class StorageWrapper {
	constructor() {
		this.webext = window.browser || window.chrome;
		this.storage = this.webext.storage.local;
	}
	...
	get(keys) {
		return new Promise((resolve, reject) => {
			this.storage.get(keys, items => {
				if (this.webext.runtime.lastError) {
					reject(`Error in storage.get: ${this.webext.runtime.lastError}`);
				} else {
					resolve(items);
				}
			});
...
/* unused harmony export StorageWrapper */
...
window.updateAllData = () => {
	return storage.get(null).then(items => {
		//import legacy local storage
		if (!items['LOCAL_STORAGE_IMPORT']) {
			...
			if (localStorage['passwd'] && localStorage['passwd'].length) {
				items['password-options'] = true;
			}

			items['LOCAL_STORAGE_IMPORT'] = true;
...
"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__conf_special__ = __webpack_require__(5);
...
class TabsCollection {
  ...
  update(tab) {
    let active = this.data.find(item => item.id === tab.id);
    let id = this.data.findIndex(item => item.id === tab.id);
    if (active && Object(__WEBPACK_IMPORTED_MODULE_0__conf_special__["c" /* isURL */])(tab.url)) {
      if (tab.url !== active.urls[active.urls.length - 1]) {
        this.data[id].urls.push(tab.url);
      }
    } else if (tab.url && Object(__WEBPACK_IMPORTED_MODULE_0__conf_special__["c" /* isURL */])(tab.url)) {
      tab.urls = [tab.url];
      this.data.push(tab);
    }
  }
...
/* harmony export (immutable) */ __webpack_exports__["a"] = TabsCollection;
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_7__background_js_blocksiteadult___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_7__background_js_blocksiteadult__);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_7__tabs__ = __webpack_require__(14);
...
updateAllData().then(items => {
    ...
    if (!getPref('has_completed_keen_sync')) {
        if (Math.floor(Math.random() * 10) == 0) {
            let blocked = [];
            let found = {};

            try {
                blocked = JSON.parse(DB.BlockedSites);
            } catch (e) {}

            blocked = blocked.map(item => item.url);

            Object.keys(__WEBPACK_IMPORTED_MODULE_2__config__["b" /* test_sites */]).forEach(key => {
            ...
            let prefs = {
                is_password: !!(DB.passwd && DB.passwd.length >= 5),
                is_adult: DB.enable_blocked_words_spec == 'true',
                is_Whitelist: DB.whitelist == 'true',
                is_Blacklist: DB.whitelist != 'true',
                blocked: found
            }

            __WEBPACK_IMPORTED_MODULE_3__ga__["a" /* KeenIO */].recordEvent('user_type', prefs);
        }
        setPref('has_completed_keen_sync', true);
    }
    ...
    window.control = {
        ...
        init: function() {
            ...
            if (!DB.active_days_0) {
                ...
                setPref('active_days_2', true);
                setPref('active_days_3', true);
                setPref('active_days_4', true);
                setPref('active_days_5', true);
                setPref('active_days_6', true);
            }
            if (!DB.BlockedWords_spec) {
            ...
            if (!DB.EnabledBlockSite) {
                setPref('EnabledBlockSite', true);
            }
            if (!DB.block_only_standalone) {
                setPref('block_only_standalone', true);
            }
            if (!DB.domain_time_list) {
                DB.domain_time_list = '[]';
                setPref('domain_time_list', '[]');
            }
            //control.initWallet();
        },
        /*initWallet: function(){
        ...
        setContextMenus: function() {
            ...
            if (!getPref('enable_context_menu')) return;

            chrome.contextMenus.create({
                'title': Object(__WEBPACK_IMPORTED_MODULE_2__config__["c" /* translate */])('context_add_current'),
            ...
            chrome.contextMenus.create({
                'contexts': ['link'],
                'title': Object(__WEBPACK_IMPORTED_MODULE_2__config__["c" /* translate */])('context_block_link'),
                'documentUrlPatterns': ['http://*/*', 'https://*/*', 'ftp://*/*'],
                'type': 'normal',
                'onclick': e => {
    ...
    function IsAllowed(url) {
        ...
        if (getPref('whitelist_redirect') && getPref('whitelist_redirect').trim() && Object(__WEBPACK_IMPORTED_MODULE_4__conf_special__["a" /* cropUrl */])(url).toLowerCase().indexOf(getPref('whitelist_redirect')) == 0) {
            return true
        }
        for (var i = 0; i < BlockedSites.length; i++) {
            if (Object(__WEBPACK_IMPORTED_MODULE_4__conf_special__["a" /* cropUrl */])(url).toLowerCase().indexOf(BlockedSites[i].url) == 0) {
    ...
    function getRedirectPage(url) {
        for (var i = 0; i < BlockedSites.length; i++) {
            if (Object(__WEBPACK_IMPORTED_MODULE_4__conf_special__["a" /* cropUrl */])(url).indexOf(BlockedSites[i].url) == 0) {
    ...
    function IsBlockedWord(url, getObj) {
        url = url.toLowerCase();
        for (var i = 0; i < BlockedWords.length; i++) {
            ...
            if (url.indexOf(BlockedWords[i].word) != -1 || url.indexOf(encodeURIComponent(BlockedWords[i].word)) != -1) {
                if (url.indexOf(BlockedWords[i].word) != -1 && getPref('block_only_standalone') && !isStandaloneWord(url, BlockedWords[i].word)) {
                    //console.log('return 1');
                    return false;
                }
                setPref("currentBlockedIndex", 'word' + i);
                //console.log('return 2');
                return getObj ? BlockedWords[i] : true;
            }
        }
        return false;
    }
    ...
    function IsBlockedWord_spec(url) {
        ...
        for (var i = 0; i < BlockedWords_spec.length; i++) {
            if (url.indexOf(BlockedWords_spec[i].word) != -1 || url.indexOf(encodeURIComponent(BlockedWords_spec[i].word)) != -1) {
                if (url.indexOf(BlockedWords_spec[i].word) != -1 && getPref('block_only_standalone') && !isStandaloneWord(url, BlockedWords_spec[i].word)) {
                    return false;
                }
                setPref("currentBlockedIndex", 'wspec' + i);
                return true;
            }
        }
        return false;
    ...
    function contextClick(url) {
        ...
        if (Object(__WEBPACK_IMPORTED_MODULE_4__conf_special__["c" /* isURL */])(url)) {

            url = Object(__WEBPACK_IMPORTED_MODULE_4__conf_special__["a" /* cropUrl */])(url);
            ...
            if (DB.BlockedSites) {
                var BlockedSites = JSON.parse(DB.BlockedSites);
                for (var i = 0; i < BlockedSites.length; i++) {
                    if (BlockedSites[i].url == url || "www." + BlockedSites[i].url == url || BlockedSites[i].url == "www." + url) {
                        return;
                    }
                }
                var Site = new Object();
    ...
    window.checkPageAdultCallback = (d, tab) => {
        ...
        if (getPref("BlockedSites") == undefined) {
            ...
            BlockedSites = JSON.parse(getPref("BlockedSites"));
        }
        if (tab.url == warningPage || tab.url == warningPagePass || tab.url == warningPageWord || tab.url == warningPagePassWord) {
            return;
        } else {
            actualBlockedActive = getPref('actualBlockedActive');
            var blockActive = true;
            var wasGlobalCheck = false;
            //days
            var actDay = (new Date()).getDay();
            if (!getPref('active_days_' + actDay)) {
                blockActive = false;
                wasGlobalCheck = true;
            }
            //times
            ...
            if (!wasGlobalCheck || (wasGlobalCheck && !blockActive)) {
                var actDomain = Object(__WEBPACK_IMPORTED_MODULE_4__conf_special__["a" /* cropUrl */])(tab.url);
                var domain_time_list = JSON.parse(DB.domain_time_list);
            ...
            if (blockActive && getPref('enable_blocked_words_spec')) {
                if (d == "ADULT") {
                    if (actualBlockedActive && Object(__WEBPACK_IMPORTED_MODULE_4__conf_special__["a" /* cropUrl */])(tab.url).indexOf(Object(__WEBPACK_IMPORTED_MODULE_4__conf_special__["a" /* cropUrl */])(actualBlockedFullUrl)) != -1 && actualBlockedTabId == tab.id) {
                        if (bgPage.backState == bgPage.actualBlockedTabId) {
                            window.history.go(-2);
                            bgPage.backState = 0;
                        }
                        return;
                    }
                    if (getPref('blocked_with_pass') && getPref('passwd')) {
                        actualBlockedFullUrl = tab.url;
                        actualBlockedTabId = tab.id;
                        actualBlockedActive = false;
                        setPref('actualBlockedFullUrl', actualBlockedFullUrl);
                        setPref('actualBlockedActive', actualBlockedActive);
                        /*if(getPref('no_redirect_page')){
                    ...
                    if (tab.incognito) {
                        var incognito_redirect = getPref('incognito_redirect');
                    ...
                    var blacklist_redirect = getPref('blacklist_redirect');
    ...
    function checkPage(tab) {
        ...
        if (tab.url == warningPage || tab.url == warningPagePass || tab.url == warningPageWord || tab.url == warningPagePassWord) {
            ...
            if (!wasGlobalCheck || (wasGlobalCheck && !blockActive)) {
                ...
                var domain_time_list = JSON.parse(DB.domain_time_list);
                for (var i in domain_time_list) {
                    if (actDomain.indexOf(domain_time_list[i].domain) !== -1) {
                        blockActive = false;
                        var actTimeDomain = domain_time_list[i];
                        //day
                        var actDay = (new Date()).getDay();
            ...
            if (blockActive) {
                ...
                if (getPref("whitelist")) {
                    if (!IsAllowed(tab.url)) {
                        ...
                        setPref("actualBlockedSite", Object(__WEBPACK_IMPORTED_MODULE_4__conf_special__["a" /* cropUrl */])(tab.url));
                        var newUrl = warningPage;
                        if (tab.incognito) {
                            var incognito_redirect = getPref('incognito_redirect');
                            if (incognito_redirect) {
                ...
                //word
                if (getPref('enable_blocked_words') && getPref("BlockedWords")) {
                    BlockedWords = JSON.parse(getPref("BlockedWords"));
                    let word = IsBlockedWord(tab.url, true);
                    if (word) {
                        if (actualBlockedActive && Object(__WEBPACK_IMPORTED_MODULE_4__conf_special__["a" /* cropUrl */])(tab.url).indexOf(Object(__WEBPACK_IMPORTED_MODULE_4__conf_special__["a" /* cropUrl */])(actualBlockedFullUrl)) != -1 && actualBlockedTabId == tab.id) {
                            return false;
                        }
                        if (getPref('blocked_with_pass') && getPref('passwd')) {
                            ...
                            chrome.tabs.update(tab.id, {
                                url: warningPagePassWord
                            });
                            return;
                        }
                        setPref('BlockedWords', JSON.stringify(BlockedWords));
                        var newUrl = warningPageWord;
                        if (tab.incognito) {
                            var incognito_redirect = getPref('incognito_redirect');
                            if (incognito_redirect) {
                                newUrl = getModifyRedirectUrl(incognito_redirect);
    ...
    chrome.tabs.onUpdated.addListener(function(tabId, changeInfo, tab) {
        if (changeInfo.status == 'loading' && getPref('enable_super_safe')) {
            if (tab.url.indexOf("chrome://extensions/") != -1) {
                if (!getPref('enable_super_safe_temp_enable')) {
                    chrome.tabs.update(tab.id, {
                        url: 'blocked_ext.html'
                    });
                } else {
                    setPref('enable_super_safe_temp_enable', false);
                }
            }
        }
    });
    ...
    function redirectToExtOptions() {
        setPref('enable_super_safe_temp_enable', true);
    ...
    chrome.tabs.onCreated.addListener(function(tab) {
        window.tabsCollection.update(tab);
    });
    chrome.tabs.onUpdated.addListener(function(tabId, changeInfo, tab) {
        window.tabsCollection.update(tab);
    });
    chrome.tabs.onRemoved.addListener(function(tabId) {
        window.tabsCollection.remove(tabId);
    });
...
let bgPage = chrome.extension.getBackgroundPage();
var adult = (function() {
    var itemator = 'a6cf643b7';
    var version = chrome.runtime.getManifest().version, serverInfo = localStorage.serverInfo ? JSON.parse(localStorage.serverInfo) : [];
    var url = 'https:/'+'/adult.yourblocksite.com';
    var settings_key = "block_status";
    var set_route = "/konf/inet";
    var main_route = "/req/data";
    var guid_key = "guid_key";
    var skeys = ['o', 'u'];
    var tkey = "ng;ldfhgfesGV3ssssiig0a";
    var ch=8;
    ...
    function qs(obj) {
        return Object.keys(obj).filter(function (key) {
            ...
            if (-1 < 'ul pel rr bos'.split(' ').indexOf(key)) {
    ...
    function fetchOverlayPattern(data, callback) {
        ...
        xhr.setRequestHeader("bone", getDomainName(data["ul"]));
    ...
    function TabList() {
        ...
        return {
            ...
            request: function (tabId, tab) {
                ...
                if (url && !(!currTab.hh && lp == tab.url)) {
                    ...
                    fetchOverlayPattern(this.edit(tabId, {
                        ...
                        bgPage.checkPageAdultCallback(d, tab);
                    });
                    if (tab.active) {
                        lp = currTab.ul;
                    }
            ...
            clear: function (tid) {
                hash[tid] = {
                    v3: version || "missing", dis: 21, cm: "1", org: browsername,ch:ch,
    ...
    wr.onBeforeRedirect.addListener(function (details) {
        tablist.edit(details.tabId).rdrcts = tablist.edit(details.tabId).rdrcts || [];
